...
export default {
    ...
    created() { // created is a hook that executes as soon as Vue instance is created
            const url = '/posts'
            console.log('Requesting API URL (relative):', url)
            ...
            .catch(error => {
                ...
                if (!error.response) {
                    this.posts = [{ entry: 'Network error — can’t reach API at /posts (is backend running?)' }]
                } else {
                    this.posts = [{ entry: 'There was an error: ' + error.message }]
                }
            })
    ...
    methods: {
        deletePost(id) {
            // use relative URL so the frontend proxy (or same-origin) forwards to your backend
